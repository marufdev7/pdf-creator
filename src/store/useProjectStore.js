...
export const useProjectStore = create((set, get) => ({
            ...
            project: touch({
                ...
                page: {
                    ...
                    margins: patch.margins
                        ? { ...state.project.page.margins, ...patch.margins }
                        : state.project.page.margins,
                },
            }),
        })),
    ...
    addSectionAfter: (sectionId, type) => {
        const section = createSection(type);
        set((state) => {
            const index = state.project.sections.findIndex(
                ...
                sections: state.project.sections.map((section) =>
                    section.id === sectionId
                        ? {
                            ...
                            ...patch,
                            data: patch.data
                                ? { ...section.data, ...patch.data }
                                : section.data,
                            style: patch.style
                                ? { ...section.style, ...patch.style }
                                : section.style,
                        }
                        : section,
                ),
    ...
    removeSection: async (sectionId) => {
        const section = get().project.sections.find(
            (item) => item.id === sectionId,
        );
        ...
        set((state) => ({
            project: touch({
                ...state.project,
                sections: state.project.sections.filter(
                    (item) => item.id !== sectionId,
                ),
            }),
        }));
        const assetStillUsed =
            assetId &&
            get().project.sections.some(
                (item) => item.type === "image" && item.data.assetId === assetId,
            );
    ...
    reorderSections: (activeId, overId) =>
        set((state) => {
            const oldIndex = state.project.sections.findIndex(
                (section) => section.id === activeId,
            );
            const newIndex = state.project.sections.findIndex(
    ...
    addImageAsset: async (file) => {
        ...
        set((state) => ({
            assets: {
                ...state.assets,
                [id]: { ...asset, url: URL.createObjectURL(file) },
            },
        }));
        return id;
    },
    ...
    removeUnusedAsset: async (assetId) => {
        if (
            ...
            get().project.sections.some(
                (section) =>
                    section.type === "image" && section.data.assetId === assetId,
            )
        )
            return;
        const asset = get().assets[assetId];
