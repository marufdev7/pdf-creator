...
import {
  ...
  readProjectPackage,
} from "../services/project/projectPackage";
...
export default function App() {
  ...
  useEffect(() => {
    async function handleClipboardPaste(event) {
      ...
      try {
        ...
        setNotice(
          `${imageFiles.length === 1 ? "Image" : `${imageFiles.length} images`} pasted as ${imageFiles.length === 1 ? "a new section" : "new sections"}.`,
        );
      } catch (error) {
        setNotice(
  ...
  async function handleWordExport() {
    ...
    try {
      ...
      setNotice(
        error instanceof Error
          ? error.message
          : "Could not export the Word document.",
      );
    } finally {
  ...
  async function handleNewProject() {
    if (
      ...
      !window.confirm(
        "Start a new project? The current project remains saved in this browser.",
      )
    )
      return;
  ...
  if (!isHydrated)
    return (
      <main className="flex min-h-screen items-center justify-center bg-canvas text-ink">
        ...
        <span className="ml-3 text-sm font-medium">Opening your workspace</span>
      </main>
    );

  const exporting = !["idle", "completed", "failed"].includes(
    exportState.status,
  );
      ...
      <header className="sticky top-0 z-40 border-b border-line bg-white/95 backdrop-blur">
        <div className="mx-auto flex min-h-16 max-w-[1800px] items-center gap-3 px-4 sm:px-6">
          ...
          <input
            ...
            className="min-w-0 flex-1 border-0 bg-transparent px-2 py-2 text-sm font-semibold text-ink outline-none sm:max-w-md"
          />
          <span
            className={`hidden text-xs sm:block ${saveError ? "text-[#a33a2c]" : "text-muted"}`}
          >
            {saveLabel}
          </span>
          <div className="flex items-center gap-1 border-l border-line pl-2">
            ...
            <input
              ref={importInputRef}
              type="file"
              accept=".pdfproj,application/octet-stream"
              onChange={(event) => handleProjectImport(event.target.files?.[0])}
              className="sr-only"
            />
            <button
              type="button"
            ...
            <button
              type="button"
              title="Export editable Word document"
              aria-label="Export editable Word document"
          ...
          <button
            ...
            className="inline-flex h-10 items-center gap-2 bg-accent px-3 text-sm font-semibold text-white transition hover:bg-[#0f5945] disabled:cursor-wait disabled:opacity-60 sm:px-4"
          >
            {exporting ? (
              <LoaderCircle size={17} className="animate-spin" />
            ) : (
        ...
        {exportState.status !== "idle" && (
          <div
            className={`border-t px-4 py-2 text-center text-xs font-medium ${exportState.status === "failed" ? "border-[#e6b7ae] bg-[#fff1ee] text-[#a33a2c]" : "border-[#cce3d9] bg-accent-soft text-accent"}`}
          >
            <span>{exportState.message}</span>
            {exporting && <span className="ml-2">{exportState.progress}%</span>}
          </div>
        )}
      </header>
      ...
      <div className="mx-auto max-w-[1800px] px-4 py-5 sm:px-6 sm:py-7">
        <div className="mb-4 flex border border-line bg-white lg:hidden">
          <button
            ...
            className={`flex h-10 flex-1 items-center justify-center gap-2 text-sm font-semibold ${mobileView === "editor" ? "bg-accent text-white" : "text-muted"}`}
          >
          ...
          <button
            ...
            className={`flex h-10 flex-1 items-center justify-center gap-2 text-sm font-semibold ${mobileView === "preview" ? "bg-accent text-white" : "text-muted"}`}
          >
            <Eye size={16} /> Preview
          </button>
        </div>
        <main className="grid items-start gap-7 lg:grid-cols-[minmax(430px,0.9fr)_minmax(520px,1.1fr)]">
          ...
          >
            <EditorPane project={project} assets={assets} actions={actions} />
          </div>
          <div
            className={mobileView === "editor" ? "hidden lg:block" : "block"}
          >
            <PreviewPane project={project} assets={assets} />
          </div>
        </main>
      </div>
      {notice && (
        <div
          role="status"
          className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 border border-line bg-ink px-4 py-3 text-sm font-medium text-white shadow-xl"
        >
          {notice}
        </div>
      )}
