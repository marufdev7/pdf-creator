...
export function estimateSectionHeight(section, contentWidth) {
  ...
  switch (section.type) {
    case "text":
      return title + countWrappedLines(section.data.body, charactersPerLine) * 25 + 30;
    case "code":
      return title + countWrappedLines(section.data.code, Math.floor(charactersPerLine * 0.85)) * 21
        + (section.data.output ? countWrappedLines(section.data.output, charactersPerLine) * 20 + 52 : 0) + 46;
    case "output":
      return title + countWrappedLines(section.data.body, charactersPerLine) * 21 + 62;
    case "image":
      return title + (section.layoutMaxHeight || 430) + (section.data.caption ? 34 : 0);
    case "chart":
  ...
  return [
    {
      ...
      data: { ...section.data, [key]: firstLines.join("\n"), lineStart, output: "" },
    },
    {
      ...section,
      ...
      data: { ...section.data, [key]: remainingLines.join("\n"), lineStart: lineStart + firstLines.length, output: section.data.output || "" },
      continuation: true,
    },
  ];
}
...
export function splitOversizedSection(section, availableHeight, contentWidth = 618) {
  if (section.type === "text") {
    const charactersPerLine = Math.max(34, Math.floor(contentWidth / 8.3));
    const maxCharacters = Math.max(140, Math.floor(Math.max(120, availableHeight - 72) / 25 * charactersPerLine));
    return chunkText(section, maxCharacters);
  }
  if (section.type === "code") return splitLinesOnce(section, "code", Math.max(5, Math.floor(Math.max(120, availableHeight - 115) / 21)));
  if (section.type === "output") return splitLinesOnce(section, "body", Math.max(5, Math.floor(Math.max(120, availableHeight - 62) / 21)));
  if (section.type === "chart" && availableHeight >= 260) {
    return [{ ...section, layoutMaxHeight: Math.max(260, Math.min(405, availableHeight - (section.title ? 38 : 0) - 12)) }];
  }
  return [section];
}

function prepareImageSection(section, assets, renderedWidth, availableHeight, layout = "portrait") {
  ...
  const portraitBudget = Math.max(220, Math.min(455, (availableHeight - SECTION_GAP - FIRST_PAGE_HEADER_HEIGHT) / 2));
  const pairedLandscapeBudget = Math.max(220, Math.min(360, availableHeight * 0.54));
  const singleLandscapeBudget = Math.max(300, Math.min(520, availableHeight * 0.72));
  const sectionBudget = layout === "landscape-pair"
    ? pairedLandscapeBudget
    : layout === "landscape-single"
      ? singleLandscapeBudget
      : portraitBudget;
  const maximumHeight = Math.max(140, sectionBudget - titleHeight - captionHeight);
  if (!asset?.width || !asset?.height) return { ...section, layoutMaxHeight: Math.min(390, maximumHeight) };
  const displayWidth = Math.min(asset.width, renderedWidth);
  const proportionalHeight = displayWidth * (asset.height / asset.width);
  return { ...section, layoutMaxHeight: Math.min(proportionalHeight, maximumHeight) };
}
...
function prepareSections(project, assets, contentWidth, availableHeight) {
  ...
  for (let index = 0; index < project.sections.length; index += 1) {
    ...
    const canPair = project.page.orientation === "landscape"
      && section.type === "image"
      && nextSection?.type === "image"
      && !nextSection.style?.pageBreakBefore;
    if (canPair) {
      const columnWidth = (contentWidth - SECTION_GAP) / 2;
      prepared.push(prepareImageSection(section, assets, columnWidth, availableHeight, "landscape-pair"));
      prepared.push(prepareImageSection(nextSection, assets, columnWidth, availableHeight, "landscape-pair"));
      index += 1;
    } else {
      const layout = project.page.orientation === "landscape" ? "landscape-single" : "portrait";
      prepared.push(prepareImageSection(section, assets, contentWidth, availableHeight, layout));
    }
  }
  return prepared;
}
...
export function paginateSections(project, assets = {}) {
  ...
  const availableHeight = dimensions.height - margins.top - margins.bottom - (project.page.showPageNumbers ? 24 : 0);
  const pages = [];
  let current = [];
  ...
  while (queue.length) {
    ...
    const requestedBreak = !original.continuation && original.style?.pageBreakBefore;
    if (requestedBreak && current.length) startPage();

    const gap = current.length ? SECTION_GAP : 0;
    const remaining = availableHeight - used - gap;
    const pairedImage = project.page.orientation === "landscape"
      && original.type === "image"
      && queue[0]?.type === "image"
    ...
    const pieces = splitOversizedSection(original, Math.max(remaining, 0), contentWidth);
    ...
    if (first !== original || pieces.length > 1) {
      ...
      current.push(first);
      used += (current.length > 1 ? SECTION_GAP : 0) + Math.min(availableHeight, firstHeight);
